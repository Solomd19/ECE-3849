 ...
 * buttons.c
 *
 *  Created on: Aug 12, 2012, modified 4/4/2021
...
void ButtonInit(void)
{
    ...
    // GPIO PK_6 = Button S2 & PK_1 = Joystick VER(Y)
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOK);
    GPIOPinTypeGPIOInput(GPIO_PORTK_BASE, GPIO_PIN_6 | GPIO_PIN_1);
    GPIOPadConfigSet(GPIO_PORTK_BASE, GPIO_PIN_6 | GPIO_PIN_1,
    GPIO_STRENGTH_2MA,
                     GPIO_PIN_TYPE_STD_WPU);

    // GPIO PD_4 = Joystick Select & PD_2 = Joystick HOR(X)
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOD);
    GPIOPinTypeGPIOInput(GPIO_PORTD_BASE, GPIO_PIN_4 | GPIO_PIN_2);
    GPIOPadConfigSet(GPIO_PORTD_BASE, GPIO_PIN_4 | GPIO_PIN_2,
    GPIO_STRENGTH_2MA,
                     GPIO_PIN_TYPE_STD_WPU);
...
void ButtonISR(void)
{
    ...
    {
        if (tic)
            gTime++; // increment time every other ISR call
        tic = !tic;
    }
}
